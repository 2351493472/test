    ...
    for epoch in range(config["meta_epochs"]):
        ...
        with torch.no_grad():
            for i, data in enumerate(tqdm(test_loader, disable=config["hide_tqdm_bar"])):
                ...
                # Loss 输出通常是 View-Major: [V0_Batch, V1_Batch ...]
                ...
                ano_map = ano_map.squeeze().cpu().numpy()  # 转 numpy 处理

                # [关键修复] Mask 必须转为 uint8 类型，否则 cv2.connectedComponents 会报错
                mask = mask.cpu().numpy().astype(np.uint8)

                img_score = torch.amax(nll, dim=(-1, -2)).cpu().numpy()

                # Accumulator 需要 Tensor
        ...
        if epoch == config["meta_epochs"] - 1:
            ...
            norms = failure_tracker.get_top_normals()
            anos = failure_tracker.get_top_anomalies()

            # 使用 0-1 范围，因为我们已经对 map 做了归一化
            visualize(norms, config["prefix"], config["class_name"], 0, 1, is_ano=False)
            visualize(anos, config["prefix"], config["class_name"], 0, 1, is_ano=True)
...
if __name__ == "__main__":
    ...
    config_obj["data_config"]["class_name"] = class_name
    ...
    if args.samplewise:
        ...
        config_obj["data_config"]["batch_size"] = 32
    else:
